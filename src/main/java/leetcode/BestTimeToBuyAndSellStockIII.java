...
public class BestTimeToBuyAndSellStockIII {
    public static void main(String[] args) {
        ...
        System.out.println(bt.maxProfit2(prices));
    }
    ...
    public int maxProfit(int[] prices) {
        if (prices == null || prices.length == 0) {
            ...
            int[] profit = new int[prices.length]; //第i天买入的最大收益
            for (int i = 0; i < prices.length - 1; i++) {//i买入
                int[] temp = new int[prices.length]; //第j天卖出的最大收益
                for (int j = i + 1; j < prices.length; j++) {//j卖出
                    int tmp = prices[j] - prices[i];
                    if (j >= prices.length - 2 && j < prices.length) {//没有下一轮
                        temp[j] = tmp;
                    } else {//可以进行两轮
                        ...
                        for (int s = j + 1; s < prices.length; s++) {
                            for (int t = s + 1; t < prices.length; t++) {
                                res[s] = Math.max(res[s], prices[t] - prices[s]);
                            }
                        }
                        int max = res[j + 1]; //求res的最大值
                        for (int p = j + 2; p < prices.length - 1; p++) {
                            if (res[p] > max) {
                                max = res[p];
                            }
                        }
                        temp[j] = Math.max(tmp, tmp + max); //从第i天买入，第j天卖出的最大收益
                    }
                }
                int max = temp[i + 1]; //计算第i天买入的最大收益
                for (int q = i + 2; q < prices.length; q++) {
                    if (temp[q] > max) {
                        max = temp[q];
                    }
                }
                profit[i] = max; //从第i天买入的最大收益
            }
            int max = profit[0];
    ...
    public int maxProfit2(int[] prices) {//允许多次购买
        if (prices.length <= 1) {
            return 0;
        } else {
            int max = 0;
    ...
    public int maxProfit3(int[] prices) {//只允许购买一次
        if (prices.length <= 1) {
            return 0;
        } else {
            int[] max=new int[prices.length];
            for(int i=0;i<prices.length-1;i++){ //第i天买入的最大收益
                int maxJ=0;
                for(int j=i+1;j<prices.length;j++){ //第j天卖出
                    if(prices[j]>maxJ){
                        maxJ=prices[j];
                    }
                }
